...
var passwordCheckOld = function () 
{
   //declares variables
    ...
    //sets the variable stringPassword to the value inputed into the html box
    stringPassword = $("password").value;
    
    //declares another variable that can perform the password check without regard to capitalization
    var stringPasswordTest = stringPassword.toLowerCase();
    
    //declares the bad password array
    ...
    //sets stringOutput to be empty for the time being
   ...
   //checks that that the password is longer than 8 characters
    ...
    {
      //checks the imputed password against the list of bad passwords
      for (var count=0; count<=arrayBadPasswords.length;count++)
         {
            if (stringPasswordTest === arrayBadPasswords[count])
            {
               stringOutput=("Uh Oh! Your password has been found on our list of bad passwords. Try a different one!");
            }
         }
    }
   // alerts the user that their password is acceptable
     if (stringOutput==="")
      {
         stringOutput = ("Excellent! That password seems fine enough to me!");
      }

//outputs the relevant alerts
$("output").value = stringOutput;

//used for testing the lowercase string password
console.log(stringPassword);
console.log(stringPasswordTest);
};

window.onload = function () 
{
   //resets the input box upon reload
   $("password").value = "";
   //no clue what this does, but I didn't write it, so i left it in
   $("password").focus();
   //runs password check program upon clicking the button
